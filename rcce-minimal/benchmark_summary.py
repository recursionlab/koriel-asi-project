...
def run_benchmark_suite():
    ...
    try:
        result = subprocess.run(
            [sys.executable, "src/minimal_rcce.py"],
            capture_output=True,
            text=True,
            cwd=".",
        )
    ...
    try:
        result = subprocess.run(
            [sys.executable, "src/test_suite.py"],
            capture_output=True,
            text=True,
            cwd=".",
        )
        if result.returncode == 0:
            ...
            for line in lines:
                if (
                    ...
                    or "Tests Passed:" in line
                    or "tokens/sec" in line
                ):
                    print(f"  {line.strip()}")
    ...
    try:
        result = subprocess.run(
            [sys.executable, "src/mathematical_benchmarks.py"],
            capture_output=True,
            text=True,
            cwd=".",
        )
    ...
    required_outputs = [
        "experiments/results/presence_cert_minimal.json",
        "experiments/results/shadow_codex_minimal.json",
    ]
    ...
    print("=" * 60)
    print(
        f"Specifications Met: {specs_passed}/{total_specs} ({specs_passed/total_specs:.1%})"
    )
    print(f"Total Execution Time: {total_time:.1f} seconds")
    print(
        f"System Status: {'FULLY COMPLIANT' if specs_passed >= total_specs-1 else 'PARTIAL COMPLIANCE'}"
    )
