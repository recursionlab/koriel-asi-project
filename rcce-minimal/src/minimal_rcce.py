...
class RCCEController:
    ...
    def __init__(self, d_model=32):
        ...
        self.xi_op = (
            np.linalg.qr(np.random.randn(d_model, d_model))[0] * 0.1
        )  # Ξ-operator
...
class PresenceCertificate:
    ...
    def generate(self, controller_state, geo_metrics):
        ...
        final_state = trace[-1]
        score = (
            0.3 * final_state["gate"]
            + 0.3 * min(1.0, final_state["ce2"])
            + 0.2 * final_state["coherence"]
            + 0.2 * final_state["ethics"]
        )
        ...
        return {
            ...
            "final_controller_state": {
                k: float(v) for k, v in final_state.items() if k != "trace"
            },
        }


def train_rcce(
    text="Consciousness emerges through recursive self-reference.",
    epochs=8,
    seq_len=12,
    lr=0.02,
):
    ...
    print("\nOutput Files:")
    print(
        f"  experiments/results/shadow_codex_minimal.json ({len(controller.state['trace'])} entries)"
    )
