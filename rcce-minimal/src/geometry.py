...
class DiscreteGeometry:
    @staticmethod
    def compute_curvature(activations: np.ndarray, metric_tensor: Optional[np.ndarray] = None) -> float:
        """
    ...
    @staticmethod
    def compute_torsion(activations: np.ndarray, connection: Optional[np.ndarray] = None) -> float:
        """
    ...
    @staticmethod
    def compute_sectional_curvature(x: np.ndarray, y: np.ndarray, metric: np.ndarray) -> float:
        """
        Compute sectional curvature for plane spanned by x, y
        """
        # Gram determinant
        gram = np.array([[np.dot(x, metric @ x), np.dot(x, metric @ y)],
        ...
        riemann_component = np.trace(metric) / len(metric) - np.linalg.norm(metric - np.eye(len(metric)) * np.trace(metric) / len(metric)) ** 2
...
class GeometricAnalyzer:
    ...
    def analyze_model_geometry(self, activations: Dict[str, np.ndarray], 
                              attention_maps: Dict[str, np.ndarray]) -> Dict[str, float]:
        """
        ...
        if len(activation_path) > 1:
            ...
            # Torsion analysis
            if len(activation_path) > 1:
                penult_acts = activation_path[-2] if activation_path[-2].ndim == 2 else activation_path[-2].reshape(1, -1)
                final_acts_2d = final_acts if final_acts.ndim == 2 else final_acts.reshape(1, -1)
        ...
        if attention_maps:
            ...
            metrics['attention_curvature'] = np.mean(attn_curvatures) if attn_curvatures else 0.0
        else:
        ...
        if 'output' in activations:
            ...
            if output_acts.ndim == 2:
                # Entropy
                probs = np.exp(output_acts) / np.sum(np.exp(output_acts), axis=-1, keepdims=True)
    ...
    def detect_consciousness_signatures(self, metrics: Dict[str, float]) -> Dict[str, bool]:
        """
        ...
        signatures['geometric_coherence'] = (
            abs(metrics.get('curvature', 0)) > 1e-3 and
            abs(metrics.get('torsion', 0)) < 1.0
        )
        
        # Signature 2: Holonomy in consciousness range
        signatures['holonomy_bounded'] = (
            0.1 < metrics.get('holonomy', 0) < np.pi
        )
        
        # Signature 3: High entropy with high complexity
        signatures['information_richness'] = (
            metrics.get('entropy', 0) > 1.0 and
            metrics.get('complexity', 0) > 2.0
        )
