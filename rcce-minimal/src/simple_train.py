...
class SimpleRCCE:
    ...
    def process(
        self, activations: Dict[str, np.ndarray], loss: float
    ) -> Dict[str, Any]:
...
def simple_train():
    ...
    for epoch in range(n_epochs):
        ...
        for i in tqdm(range(len(tokens) - 8), desc=f"Epoch {epoch+1}"):
            ...
            if i % 5 == 0:  # Update every 5 steps
                ...
                probs = np.exp(pred_logits) / np.sum(
                ...
                model.W_out -= (
                    learning_rate
                    * control["learning_rate_modifier"]
                    * (h.T @ grad_logits)
                )
    ...
    print("\nFiles generated:")
    print(
        f"  experiments/results/shadow_codex.json ({len(controller.trace)} trace entries)"
    )
