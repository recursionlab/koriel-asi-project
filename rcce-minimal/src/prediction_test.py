...
def test_predictions():
    ...
    for i in range(5):  # Predict 5 more characters
        ...
        try:
            predicted_char = (
                chr(predicted_token) if 32 <= predicted_token <= 126 else " "
            )
        except Exception:
            predicted_char = " "
    ...
    for test_case in test_cases:
        ...
        print(
            f"  '{test_case}' -> confidence: {max_confidence:.3f}, entropy: {entropy:.2f}"
        )
...
def analyze_prediction_types():
    ...
    for category, test_input in test_categories.items():
        ...
        print(
            f"  {category}: printable={printable_mass:.2f}, letters={letter_mass:.2f}, gate={controller.state['gate']:.3f}"
        )
