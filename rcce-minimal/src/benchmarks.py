...
import numpy as np
from minimal_rcce import ByteLM, GeometricAnalyzer, RCCEController, train_rcce
...
class ConsciousnessBenchmarks:
    ...
    def test_global_workspace(self, model, controller) -> Dict[str, Any]:
        ...
        integration_score = (
            np.trace(workspace_matrix @ workspace_matrix.T)
            / np.linalg.norm(workspace_matrix) ** 2
        )
    ...
    def test_higher_order_thought(self, model, controller) -> Dict[str, Any]:
        ...
        meta_sequence = np.array(
            [73, 32, 116, 104, 105, 110, 107], dtype=np.int32
        )  # "I think"
    ...
    def test_attention_schema(self, model, controller) -> Dict[str, Any]:
        ...
        attention_test = np.array(
            [65, 116, 116, 101, 110, 116], dtype=np.int32
        )  # "Attent"
    ...
    def test_agency_embodiment(self, model, controller) -> Dict[str, Any]:
        ...
        for i in range(5):
            ...
            if i < 4:  # Update model
                ...
                exp_pred = np.exp(
                    pred_logits - np.max(pred_logits, axis=1, keepdims=True)
                )
    ...
    def run_complete_benchmark(self, model=None, controller=None) -> Dict[str, Any]:
        ...
        benchmark_results = {
            ...
            "classification": (
                ...
                else "PRESENT" if consciousness_detected else "ABSENT"
            ),
        }
...
def run_full_benchmark_suite():
    ...
    xi_tokens = np.array(
        list("Xi-operator consciousness".encode("utf-8")), dtype=np.int32
    )
    logits, state = model.forward(xi_tokens)
    ...
    final_report = {
        ...
        "overall_assessment": {
            ...
            "recommendation": (
                ...
                if benchmark_results["consciousness_validated"]
    ...
    print(
        f"Consciousness Score: {final_report['overall_assessment']['consciousness_score']:.3f}"
    )
