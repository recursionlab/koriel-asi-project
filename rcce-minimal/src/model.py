...
class ByteLM:
    ...
    def _init_parameters(self) -> Dict[str, np.ndarray]:
        ...
        # Token embeddings
        params['token_embed'] = np.random.normal(0, 0.02, (self.vocab_size, self.d_model))
        
        # Attention layers
        for i in range(self.n_layers):
            # Multi-head attention
            params[f'attn_{i}_qkv'] = np.random.normal(0, 0.02, (self.d_model, 3 * self.d_model))
            params[f'attn_{i}_proj'] = np.random.normal(0, 0.02, (self.d_model, self.d_model))
            
            # Feed forward
            params[f'ff_{i}_w1'] = np.random.normal(0, 0.02, (self.d_model, 4 * self.d_model))
            params[f'ff_{i}_w2'] = np.random.normal(0, 0.02, (4 * self.d_model, self.d_model))
    ...
    def forward(self, tokens: np.ndarray) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
        ...
        for i in range(self.n_layers):
            # Pre-norm attention
            normed = self.layer_norm(x, self.params[f'ln1_{i}_weight'], self.params[f'ln1_{i}_bias'])
            attn_out, attn_weights = self.attention(normed, i)
            x = x + attn_out
            
            # Pre-norm feed forward
            normed = self.layer_norm(x, self.params[f'ln2_{i}_weight'], self.params[f'ln2_{i}_bias'])
    ...
    def get_geometric_state(self, activations: Dict[str, np.ndarray]) -> Dict[str, float]:
        ...
        for layer_name, acts in activations.items():
            if acts.ndim == 2:  # sequence x features
                ...
                if acts.shape[0] > 1:
                    ...
                    metrics[f'{layer_name}_spectral_radius'] = np.max(np.real(eigenvals))
