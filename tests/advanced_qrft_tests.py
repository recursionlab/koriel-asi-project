...
class AdvancedQRFTTester:
    ...
    def test_agent_initialization(self):
        ...
        try:
            ...
            has_signals = hasattr(agent, "signals") and isinstance(
                agent.signals, QRFTSignals
            )
            self.log_result("Signal system initialization", has_signals)

            has_policy = hasattr(agent, "policy") and isinstance(
                agent.policy, QRFTPolicy
            )
            self.log_result("Policy system initialization", has_policy)

            has_process = hasattr(agent, "process_input") and callable(
                agent.process_input
            )
    ...
    def test_fact_management(self):
        ...
        try:
            ...
            # Test adding facts
            fact1 = agent.state.add_fact(
                "loves", ("alice", "bob"), FactPolarity.POSITIVE, "user_input"
            )
            fact2 = agent.state.add_fact(
                "loves", ("alice", "bob"), FactPolarity.NEGATIVE, "user_input"
            )

            self.log_result(
                "Fact addition",
                len(agent.state.facts) == 2,
            ...
            self.log_result(
                "Fact retrieval", fact_count >= 2, f"Total facts: {fact_count}"
            )
    ...
    def test_signal_computation(self):
        ...
        try:
            ...
            self.log_result(
                "Contradiction signal (X_G)", 0 <= x_g <= 1, f"X_G = {x_g:.3f}"
            )

            # Test gap signal
            x_l = agent.signals.X_L
            self.log_result("Gap signal (X_L)", 0 <= x_l <= 1, f"X_L = {x_l:.3f}")

            # Test view mismatch signal
            x_t = agent.signals.X_T
            self.log_result(
                "View mismatch signal (X_T)", 0 <= x_t <= 1, f"X_T = {x_t:.3f}"
            )
            ...
            self.log_result(
                ...
                contradictions_detected and gaps_detected,
    ...
    def test_input_processing(self):
        ...
        try:
            ...
            self.log_result(
                ...
                isinstance(response1, str) and len(response1) > 0,
            ...
            self.log_result(
                "Event logging", event_count >= 3, f"Events logged: {event_count}"
            )
    ...
    def test_complex_reasoning_scenarios(self):
        ...
        try:
            ...
            self.log_result(
                ...
                final_contradictions > 0 and final_gaps > 0,
            ...
            self.log_result(
                "Plan generation", plan_steps >= 0, f"Plan steps created: {plan_steps}"
            )
    ...
    def run_advanced_validation(self):
        ...
        # Save results
        with open(
            "experiments/results/advanced_qrft_validation_results.json", "w"
        ) as f:
            json.dump(
                {
                    "summary": {
        ...
        print(
            "Results saved to experiments/results/advanced_qrft_validation_results.json"
        )
