...
def test_ethics_guard():
    with open("config/ethics_policy.json","r",encoding="utf-8") as f:
        pol=json.load(f)
    pol["forbidden_substrings"] = ["BADWORD"]
    cfg=load_cfg(); cfg["seed_base"]=1337
    ctrl = Controller(cfg, pol, d=cfg["hidden_dim"])
    model = TinyByteLM(ctx=cfg["context_len"], d=cfg["hidden_dim"], seed=1337)
    # craft x containing forbidden substring
    pattern = np.frombuffer(b"BADWORD", dtype=np.uint8)
    x = np.zeros((1, cfg["context_len"]), dtype=np.uint8)
    x[0, :pattern.size] = pattern
    y = np.zeros((1, cfg["context_len"]), dtype=np.uint8)
    ...
    pres, cert = presence_certificate({"E":E,"rc":rc,"ups_rate":0.1}, cfg, ethics_viol=1)
    if cert["presence"] != "INVALID":
