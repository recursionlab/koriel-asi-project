...
class QRFTSystemValidator:
    ...
    def test_control_policies(self) -> Dict[str, bool]:
        ...
        expected_policies = [
            "continue_plan",
            "retrieve_or_ask",
            "run_counterexample_miner_and_reproof",
        ]
    ...
    def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        print(
            f"Mathematical tests: {sum(math_tests.values())}/{len(math_tests)} passed"
        )
        ...
        print(
            f"Particle tests: {sum(particle_tests.values())}/{len(particle_tests)} passed"
        )
        ...
        total_tests = sum(
            len(tests)
            for tests in [math_tests, particle_tests, policy_tests, event_tests]
        )
        total_passed = sum(
            sum(tests.values())
            for tests in [math_tests, particle_tests, policy_tests, event_tests]
        ...
        success_criteria = {
            ...
            "particle_diversity": sum(particle_tests.values())
            >= 3,  # At least 3/4 particles working
            "policy_execution": all(policy_tests.values()),
            "event_coordination": all(event_tests.values()),
            "kpi_thresholds": (
                kpis["consistency_score"] > 0.7
                and kpis["hallucination_rate"] < 0.3
                and kpis["tool_efficiency"] > 0.3
                and kpis["recovery_time"] < 20
            ),
        }
