...
class GlitchonCritic:
    ...
    def detect_contradictions(self, 
                            ...
                            external_context: Optional[Dict[str, Any]] = None) -> ContradictionState:
        ...
        if external_context:
            external_contradictions = self._check_external_validation(statements, external_context)
        ...
        critical_failures = [c for c in contradictions if c.severity > self.severity_threshold]
            ...
            for j, stmt2 in enumerate(statements[i+1:], i+1):
                ...
                for pattern_name, pattern_info in self.contradiction_patterns.items():
                    ...
                    if contradiction:
                        ...
                        contradiction.contradiction_type = ContradictionType.SELF_CONSISTENCY
                        contradictions.append(contradiction)
                        
        return contradictions
        
    def _check_unit_tests(self, statements: List[str], test_results: Dict[str, bool]) -> List[Contradiction]:
        """Check unit test failures against statements"""
        contradictions = []
        
        failed_tests = {name: result for name, result in test_results.items() if not result}
        
        for test_name, _ in failed_tests.items():
            # Find statements that might relate to this test
            related_statements = self._find_test_related_statements(test_name, statements)
    ...
    def _check_external_validation(self, statements: List[str], 
                                 external_context: Dict[str, Any]) -> List[Contradiction]:
    ...
    def _detect_negation_contradiction(self, stmt1: str, stmt2: str, 
                                     pos1: int, pos2: int) -> Optional[Contradiction]:
    ...
    def _detect_numerical_contradiction(self, stmt1: str, stmt2: str,
                                      pos1: int, pos2: int) -> Optional[Contradiction]:
        ...
        for entity, values1 in numbers1.items():
            if entity in numbers2:
                ...
                if any(v1 != v2 for v1 in values1 for v2 in values2):
                    confidence = 0.8
                    severity = min(abs(max(values1) - max(values2)) / max(max(values1), max(values2)), 1.0)
    ...
    def _detect_temporal_contradiction(self, stmt1: str, stmt2: str,
                                     pos1: int, pos2: int) -> Optional[Contradiction]:
        ...
        temporal_contradiction = self._check_temporal_conflict(time1, time2, stmt1, stmt2)
    ...
    def _detect_causality_violation(self, stmt1: str, stmt2: str,
                                  pos1: int, pos2: int) -> Optional[Contradiction]:
    ...
    def _detect_definition_inconsistency(self, stmt1: str, stmt2: str,
                                       pos1: int, pos2: int) -> Optional[Contradiction]:
        ...
        if def1['entity'] == def2['entity'] and def1['definition'] != def2['definition']:
            return Contradiction(
                contradiction_type=ContradictionType.LOGICAL_CONTRADICTION,
                confidence=0.8,
                severity=0.9,
    ...
    def _check_temporal_conflict(self, time1: Dict, time2: Dict, stmt1: str, stmt2: str) -> Optional[Dict]:
    ...
    def _check_causal_consistency(self, causal1: List[Dict], causal2: List[Dict]) -> Optional[Dict]:
    ...
    def _filter_contradictions(self, contradictions: List[Contradiction]) -> List[Contradiction]:
    ...
    def _find_test_related_statements(self, test_name: str, 
                                    statements: List[str]) -> List[Tuple[int, str]]:
    ...
    def _validate_against_fact(self, statements: List[str], fact: Dict) -> List[Contradiction]:
    ...
    def generate_counterexamples(self, contradictions: List[Contradiction]) -> List[Dict[str, Any]]:
