"""Safety utilities with graceful fallback when ``psutil`` is unavailable."""
...
try:  # psutil is optional in minimal environments
...
class ResourceMonitor:
    ...
    def check_limits(self) -> Dict[str, Any]:
        ...
        if elapsed_time > self.limits.max_execution_time:
            violations.append(
                f"Time limit exceeded: {elapsed_time:.1f}s > {self.limits.max_execution_time}s"
            )

        # Check memory limit
        if self.process and current_memory > self.limits.max_memory_mb:
            violations.append(
                f"Memory limit exceeded: {current_memory:.1f}MB > {self.limits.max_memory_mb}MB"
            )
...
class ExperimentSafetyGate:
    ...
    def check_safety_requirements(
        self, experiment_config: Dict[str, Any], allow_experiments: bool = False
    ) -> Dict[str, Any]:
...
def check_system_resources() -> Dict[str, Any]:
    ...
    else:  # fallback using standard library only
        import shutil

        disk = shutil.disk_usage("/")
        cpu_count = os.cpu_count() or 0
        mem_total_gb = mem_available_gb = mem_percent = 0.0
