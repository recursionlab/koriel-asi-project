...
class KorielOperator:
    ...
    def measure_uncoherence(self, state: KorielState) -> UncoherenceMetrics:
        ...
        gradient_norm = float(self.alpha * np.linalg.norm(state.velocity))

        # β·paradox(s) - contradiction detection |T∩F|
        paradox_level = float(self.beta * self._detect_paradox(state))

        # γ·drift_G(s) - deviation from goal manifold
        manifold_drift = float(self.gamma * self.goal_manifold.drift_from_goal(state))

        # η·|holonomy(s)| - path-dependent inconsistency
        holonomy = float(self.eta * abs(self._compute_holonomy(state)))

        total = float(gradient_norm + paradox_level + manifold_drift + holonomy)

        # Return complete UncoherenceMetrics object
    ...
    def _compute_holonomy(self, state: KorielState) -> float:
        ...
        for i in range(2, len(self.history)):
            ...
            if (
                np.linalg.norm(v1) > 0
                and np.linalg.norm(v2) > 0
                and np.linalg.norm(v3) > 0
            ):
    ...
    def detect_stall(self, state: KorielState) -> bool:
        ...
        if (
            np.linalg.norm(projected_gradient) < self.tau_gradient
            and current_u > self.tau_uncoherence
        ):
            return True

        return False

    def apply_koriel_lift(
        self, state: KorielState, direction: np.ndarray
    ...
    def _assess_step_risk(
        self, state: KorielState, direction: np.ndarray, stride: float
    ) -> float:
        ...
        try:
            ...
            # Risk factors
            feasibility_risk = (
                0.0 if self.goal_manifold.is_feasible(next_state) else 10.0
            )
    ...
    def cache_if_risky(
        self, state: KorielState, risk: float, threshold: float = 1.0
    ) -> None:
    ...
    def step(self, state: KorielState) -> KorielStep:
        ...
        if stall_detected:
            # Apply Koriel-lift
            projected_direction, carrier_used = self.apply_koriel_lift(
                state, projected_direction
            )
            lift_applied = True
    ...
    def run_sequence(
        self,
        initial_state: KorielState,
        max_steps: int = 100,
        target_uncoherence: float = 0.01,
    ) -> List[KorielStep]:
    ...
    def get_status_report(self) -> Dict[str, Any]:
        ...
        lift_usage = sum(1 for step in recent_steps if step.lift_applied) / len(
            recent_steps
        )
