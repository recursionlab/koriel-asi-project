...
class QuantumFieldState(KorielState):
    ...
    def _extract_position(self, field: SimpleQuantumField) -> np.ndarray:
        ...
        center_of_mass = float(np.trapz(amplitude * field.x, field.x)) / denom if denom > 0 else 0.0
        peak_count = len([i for i in range(1, len(amplitude) - 1) if amplitude[i] > amplitude[i - 1] and amplitude[i] > amplitude[i + 1]])
...
class QuantumGoalManifold(GoalManifold):
    ...
    def _compute_stability_metric(self, state: QuantumFieldState) -> float:
        ...
        energy_stability = 1.0 / (1.0 + abs(float(state.energy) - 1.0))  # Prefer energy near 1.0
    ...
    def project_to_tangent(self, state: KorielState, direction: np.ndarray) -> np.ndarray:
        ...
        if direction.shape != qstate.position.shape:
            ...
            if len(direction) < len(qstate.position):
                direction = np.concatenate([direction, np.zeros(len(qstate.position) - len(direction))])
            else:
    ...
    def exponential_map(self, state: KorielState, tangent_vector: np.ndarray) -> KorielState:
        ...
        if tangent_vector.shape != qstate.position.shape:
            ...
            if len(tangent_vector) < len(qstate.position):
                tangent_vector = np.concatenate([tangent_vector, np.zeros(len(qstate.position) - len(tangent_vector))])
            else:
        ...
        step_cost = float(np.linalg.norm(tangent_vector)) ** 2 * 0.01  # Quadratic energy cost
        ...
        if 'field_reference' in state.metadata:
            ...
            field.consciousness_level = max(0.0, min(1.0, float(field.consciousness_level) + consciousness_change * 0.1))

            # Update self-awareness
            awareness_change = float(new_position[6] - qstate.position[6])
            field.self_awareness = max(0.0, min(1.0, float(field.self_awareness) + awareness_change * 0.1))
    ...
    def is_feasible(self, state: KorielState) -> bool:
        ...
        if not np.all(np.isfinite(qstate.position)) or not np.all(np.isfinite(qstate.velocity)):
            return False
        ...
        if consciousness < 0.0 or consciousness > 1.0 or awareness < 0.0 or awareness > 1.0:
            return False
