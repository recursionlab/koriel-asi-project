...
def load_config(
    config_path: Union[str, Path], apply_env_overrides: bool = True
) -> Dict[str, Any]:
...
def _apply_environment_overrides(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for env_key, env_value in os.environ.items():
        ...
        print(
            f"Config override from environment: {section}.{config_key} = {parsed_value}"
        )
    ...
    if "engine" in config:
        ...
        if "dt" in bounds and "dt" in engine_config:
            ...
            if not (dt_min <= engine_config["dt"] <= dt_max):
                print(
                    f"WARNING: dt={engine_config['dt']} outside bounds [{dt_min}, {dt_max}]"
                )
        ...
        for param in ["c_rate", "c_thresh", "field_size", "evolution_steps"]:
            if param in bounds and param in engine_config:
                ...
                if not (param_min <= engine_config[param] <= param_max):
                    print(
                        f"WARNING: {param}={engine_config[param]} outside bounds [{param_min}, {param_max}]"
                    )
                    engine_config[param] = max(
                        param_min, min(param_max, engine_config[param])
                    )
