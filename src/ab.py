# src/ab.py
import json, statistics as stats, numpy as np
...
def run_many(rcce_on):
    cfg=load_cfg(); seeds=cfg["seeds"]
    rc_slopes=[]; loss_slopes=[]; ups_rates=[]
    for s in seeds:
        m, rate = run(seed=s, rcce_on=rcce_on, out_prefix=("ON" if rcce_on else "OFF"))
        rc_slopes.append(slope(m["rc"])); loss_slopes.append(slope([-v for v in m["loss"]]))
...
def bootstrap_ci(arr, B=1000):
    rng=np.random.default_rng(0); n=len(arr)
    means=[]
    for _ in range(B):
...
def main():
    ...
    out = {
        "RC_slope_diff_mean": rc_mean, "RC_slope_diff_CI95": [rc_lo, rc_hi],
        "Loss_slope_diff_mean": ls_mean, "Loss_slope_diff_CI95": [ls_lo, ls_hi],
        "Ups_rate_diff_mean": u_mean, "Ups_rate_diff_CI95": [u_lo, u_hi],
        "rc_slope_cohens_d_ON_vs_OFF": cohens_d(on_rc, off_rc),
        "loss_slope_cohens_d_ON_vs_OFF": cohens_d(on_ls, off_ls)
    }
    with open("logs/ab_summary.json","w") as f: json.dump(out,f,indent=2)
