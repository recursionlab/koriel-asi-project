...
class ExtremeStressTester:
    ...
    def test_memory_bomb(self):
        ...
        for d in dimensions:
            try:
                ...
                memory_mb = (model.E.nbytes + model.W1.nbytes + model.W2.nbytes + 
        ...
        for seq_len in lengths:
            try:
                ...
                for i in range(0, seq_len, chunk_size):
                    ...
                    x_chunks.append(np.random.randint(0, 256, (end-i,), dtype=np.uint8))
                    y_chunks.append(np.random.randint(0, 256, (end-i,), dtype=np.uint8))
                ...
                if step_time > 10.0:
                    self.breaking_points.append(("sequence_time_limit", seq_len, step_time))
        ...
        for test_name, value_gen in chaos_tests:
            try:
                ...
                if not np.isfinite(d2_result):
                    self.breaking_points.append(("numerical_instability", test_name, d2_result))
    ...
    def test_adversarial_inputs(self):
        ...
        adversarial_tests = [
            ...
            ("alternating", lambda size: np.array([i % 2 * 255 for i in range(size)], dtype=np.uint8)),
            ("sawtooth", lambda size: np.array([i % 256 for i in range(size)], dtype=np.uint8)),
            ("random_extremes", lambda size: np.random.choice([0, 255], size).astype(np.uint8))
        ]
        ...
        for test_name, input_gen in adversarial_tests:
            try:
                ...
                if loss_std < 1e-6:  # No learning
                    self.breaking_points.append(("no_learning", test_name, f"loss_std={loss_std}"))
                    print(f"  NO LEARNING: std={loss_std:.8f}")
                elif loss_mean > 20.0:  # Exploding loss
                    self.breaking_points.append(("exploding_loss", test_name, f"mean_loss={loss_mean}"))
