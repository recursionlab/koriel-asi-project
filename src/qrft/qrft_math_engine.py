...
class QRFTMathEngine:
    ...
    def __init__(self):
        ...
        self.transformations = transformations[0], transformations[1], transformations[2]
    ...
    def _parse_natural_language(self, query: str) -> Optional[MathTask]:
        ...
        if re.search(r"differentiate|derivative|d/dx", query):
            expr_match = re.search(r"(?:differentiate|derivative|d/dx)\s+(.+?)(?:\s+with\s+respect\s+to\s+(\w+))?", query)
            if expr_match:
                return MathTask(
                    task_type=MathTaskType.DIFFERENTIATE,
                    expression=expr_match.group(1),
                    variable=expr_match.group(2) if len(expr_match.groups()) > 1 else 'x'
                )
        
        # Integrate patterns
        if re.search(r"integrate|integral", query):
            expr_match = re.search(r"(?:integrate|integral)\s+(.+?)(?:\s+(?:with\s+respect\s+to\s+|d)(\w+))?", query)
            if expr_match:
                return MathTask(
        ...
        # Limit patterns
        limit_match = re.search(r"limit\s+of\s+(.+?)\s+as\s+(\w+)\s+(?:approaches|goes\s+to|\->)\s+(.+)", query)
        if limit_match:
            return MathTask(
        ...
        if re.search(r"simplify", query):
            ...
            if expr_match:
                return MathTask(
                    task_type=MathTaskType.SIMPLIFY,
                    expression=expr_match.group(1)
                )
    ...
    def solve_equation_system(self, equations: List[str], variables: List[str]) -> MathResult:
