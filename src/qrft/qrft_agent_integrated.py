...
from .qrft_agent_core import FactPolarity, QRFTAgent
from .qrft_reasoners import Document, ReasonerHub
...
class IntegratedQRFTAgent(QRFTAgent):
    ...
    def _execute_compute(self, expression: str) -> str:
        ...
        if "=" in expression:
            ...
            if len(parts) == 2:
                ...
                variables = [
                    v for v in variables if v not in ["sin", "cos", "tan", "log", "exp"]
                ]
                ...
                if variables:
                    ...
                    if solutions:
                        ...
                        return (
                            self.templates["compute"].format(
                                operation="solve", expression=f"{expression} for {var}"
                            )
                            + f"\nSolutions: {solutions}"
                        )
            ...
            if result:
                ...
                return (
                    self.templates["compute"].format(
                        operation="differentiate", expression=func_part
                    )
                    + f"\nResult: {result}"
                )
            ...
            if result:
                ...
                return (
                    self.templates["compute"].format(
                        operation="simplify", expression=expression
                    )
                    + f"\nResult: {result}"
                )
    ...
    def _execute_counterexample(self, claim: str) -> str:
        ...
        if contradictions:
            ...
            if counterexamples:
                ...
                return (
                    self.templates["counterexample"].format(
            ...
            if counterexamples:
                return (
                    self.templates["counterexample"].format(target=claim)
    ...
    def _execute_view_shift(self, context: str) -> str:
        ...
        if current_gaps > current_facts:
            ...
            return (
                self.templates["view_shift"].format(target="broader information search")
            ...
            return (
                self.templates["view_shift"].format(target="information synthesis")
            ...
            return (
                self.templates["view_shift"].format(
        ...
        for pattern in equation_patterns:
            ...
            for match in matches:
                self.state.add_plan_step(
                    action="compute", target=match.strip(), params={"type": "equation"}
                )
