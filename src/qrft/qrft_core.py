...
class QRFTRuntime:
    ...
    def __init__(self, config: QRFTConfig):
        ...
        if abs(config.gamma) >= 1.0:
            raise ValueError(f"Stability condition violated: |γ|={abs(config.gamma)} ≥ 1")
    ...
    def initialize_state(self, S_init: np.ndarray, Lambda_init: np.ndarray, t: float = 0.0):
    ...
    def _compute_invariants(self, state: QRFTState) -> Dict[str, float]:
        ...
        box_Lambda = float(np.mean(np.gradient(np.gradient(Lambda)))) if Lambda.ndim > 0 else 0.0
    ...
    def _compute_triggers(self, invariants: Dict[str, float]) -> Dict[ParticleType, float]:
        ...
        rho = invariants['I_cross']**2 / ((invariants['I_S'] + delta) * (invariants['I_Lambda'] + delta))
    ...
    def _compute_sources(self, triggers: Dict[ParticleType, float]) -> Dict[ParticleType, float]:
    ...
    def step(self, dt: float = 0.01) -> Dict[str, any]:
        ...
        ddS_dt2 = -self.cfg.m_S**2 * self.state.S - gamma_damp * np.gradient(self.state.S)
        ddLambda_dt2 = -self.cfg.m_Lambda**2 * self.state.Lambda - gamma_damp * np.gradient(self.state.Lambda)
    ...
    def _estimate_entropy(self) -> float:
        ...
        grad_Lambda = np.gradient(self.state.Lambda) if self.state.Lambda.ndim > 0 else np.array([0.0])
