...
class QRFT1DSimulator:
    ...
    def __init__(self, config: QRFTConfig1D):
        ...
        self.K = np.array([[1.0, config.gamma], [config.gamma, 1.0]])
        self.K_inv = np.linalg.inv(self.K)

        # Mass matrix M²
        self.M2 = np.array([[config.mS**2, 0], [0, config.mL**2]])
    ...
    def compute_invariants(
        self, S: np.ndarray, Lambda: np.ndarray
    ) -> Dict[str, np.ndarray]:
        ...
        box_S = self.spatial_derivative_2nd(
            S
        )  # ∂²S/∂x² (time part handled in evolution)
    ...
    def compute_triggers(
        self, invariants: Dict[str, np.ndarray]
    ) -> Dict[str, np.ndarray]:
        ...
        return {"X_G": X_G, "X_F": X_F, "X_T": X_T, "X_R": X_R, "rho": rho}
    ...
    def detect_events(
        self, t: float, triggers: Dict[str, np.ndarray], sources: Dict[str, np.ndarray]
    ):
    ...
    def compute_energy(self, t_idx: int) -> float:
        ...
        gradient = 0.5 * (
            dS_dx**2 + 2 * self.cfg.gamma * dS_dx * dLambda_dx + dLambda_dx**2
        )
        ...
        for n in range(self.Nt - 1):
            ...
            if n % save_every == 0 or n == self.Nt - 2:
                ...
                if n % (save_every * 10) == 0:
                    ...
                    print(
                        f"  t={t+self.dt:.2f}: Energy={energy:.6f}, drift={energy_drift:.2e}"
                    )
...
def run_example_simulation():
    ...
    x = sim.x
    S_init = np.exp(-(((x - 2.0) / 0.5) ** 2)) + np.exp(-(((x - 6.0) / 0.5) ** 2))
