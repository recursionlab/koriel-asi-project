...
def generate_with_rcce(
    model, prompt_tokens, max_tokens=400, temperature=0.9, device="cpu"
):
    ...
    if isinstance(prompt_tokens, list):
        tokens = torch.tensor(prompt_tokens, dtype=torch.long, device=device).unsqueeze(
            0
        )
    else:
    ...
    with torch.no_grad():
        for i in range(max_tokens):
            ...
            if len(attn_history) >= 2:
                ...
                from rcc.controller import phi22_route, xi_reflect
