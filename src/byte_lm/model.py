...
class TinyByteTransformer(nn.Module):
    def __init__(self, vocab_size=256, d_model=128, n_head=4, n_layer=2, seq_len=128, dropout=0.1):
        super().__init__()
    ...
    def forward(self, x, return_attn=True):
        B, T = x.shape
        assert T <= self.seq_len, f"Cannot forward sequence of length {T}, max is {self.seq_len}"
