...
def run_basic_demo(seed: int, output_dir: Path):
    ...
    try:
        ...
        run_rcce_demo(
            seed=seed, T=80, N=32, Dval=16, Nt=4
        )  # Smaller params for smoke test
...
def generate_results(demo_results: dict, output_dir: Path):
    ...
    results = {
        ...
        "validation": {
            "rc_improvement": demo_results.get("final_RC", 0)
            > demo_results.get("metrics_summary", {}).get("RC", 0) * 0.8,
...
def main():
    ...
    parser.add_argument(
        "--task", required=True, help="Demo task to run (basic, advanced, etc.)"
    )
    parser.add_argument(
