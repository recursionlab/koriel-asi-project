...
def run_demo(seed=7, T=160, N=64, Dval=24, Nt=8):
    ...
    k1, k2, k3 = 0.9, 0.2, 0.5           # observer gains
    lam_cost, mu_incoh, nu_eth = 0.5, 0.6, 2.0

    V = np.random.randn(N, Dval) / np.sqrt(Dval)
    prime_basis = np.random.randn(Dval); prime_basis /= (np.linalg.norm(prime_basis)+1e-12)

    goal_center = np.random.randint(0, N)
    hat_a = np.exp(-0.5*((np.arange(N)-goal_center)/6.0)**2); hat_a /= hat_a.sum()
    ...
    tau = 1.0; b = np.zeros(N); m = np.zeros(N); s_phase = +1
    a_prev = np.ones(N)/N; D_prev = 0.0
    v_prev = np.zeros(Dval); S_prev = agg_topics(a_prev)

    win = deque(maxlen=16); hol_prev = 0.0
    rows = []
    ...
    def lambda_plus_reinject(bias, a):
        ...
        delta = np.zeros_like(bias); delta[low_idx] = 0.15
        return bias + delta
    ...
    for t in range(T):
        ...
        # metrics
        D_t = kl(a_t, a_prev); dD = D_t - D_prev; H_t = entropy(a_t)
        v_t = V.T @ a_t; S_t = agg_topics(a_t)
        RC_t = (
        ...
        pi = 0.8*a_t + 0.2*np.random.dirichlet(np.ones(N)); pi /= pi.sum()
        ...
        rows.append(dict(t=t, D=D_t, dD=dD, H=H_t, C=C_t, RC=RC_t, E=E_t, ZI=ZI_t,
                         tau=tau, ce2=ce2_score, ethic=ethic_cost, Y=int(Y_fired)))
        a_prev, D_prev, v_prev, S_prev = a_t, D_t, v_t, S_t

    df = pd.DataFrame(rows)
    out_csv = "rcce_run_metrics.csv"; df.to_csv(out_csv, index=False)
    ...
    for key, title in [
        ...
        plt.title(title); plt.xlabel("t"); plt.ylabel(key)
        plt.show()
