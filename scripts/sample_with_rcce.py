...
def main():
    ...
    parser.add_argument(
        "--tokens", type=int, default=400, help="Number of tokens to generate"
    )
    parser.add_argument(
        "--temperature", type=float, default=0.9, help="Sampling temperature"
    )
    parser.add_argument(
        "--prompt", type=str, default="The recursive", help="Text prompt"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="checkpoints/tinylm.pt",
    ...
    output_tokens, metrics_log = generate_with_rcce(
        model, prompt_tokens, max_tokens=args.tokens, temperature=args.temperature
    )
