...
def showcase_contradiction_handling():
    ...
    for i, (pos_stmt, neg_stmt) in enumerate(contradictory_pairs, 1):
        ...
        print(f"   Signals: X_G={signals.X_G:.3f} X_L={signals.X_L:.3f} X_F={signals.X_F:.3f} X_T={signals.X_T:.3f}")
...
def showcase_signal_computation():
    ...
    scenarios = [
        ...
        },
        {
            'name': 'Novel Query',
            'actions': [
                lambda: agent.process_input("What about quantum entanglement in parallel universes?")
            ],
...
def showcase_complete_system():
    ...
    print(f"Total reasoning chains: {state_summary['reasoning_chains']['total_chains']}")
...
def generate_performance_report(agents: List[QRFTAgent]) -> Dict[str, Any]:
    ...
    total_contradictions = sum(len(agent.state.get_contradictions()) for agent in agents)
    total_chains = sum(agent.state.get_reasoning_chain_summary()['total_chains'] for agent in agents)
    ...
    return {
        ...
        'math_engine_working': agents[0].math_engine is not None and agents[0].math_engine.available if agents else False,
...
def main():
    ...
    if (report['math_engine_working'] and 
        report['all_signals_implemented'] and
        report['reasoning_chain_tracking'] and
        report['enhanced_contradiction_detection']):
