...
The agent uses:
  ...
  • QRFT signals for autonomous reasoning control
    ...
    def show_debug_info(self, duration: float):
        ...
        print(
            f"State: {len(state.facts)} facts, {len(state.gaps)} gaps, {len(state.plan_steps)} plan steps"
        )
    ...
    def reset_agent(self):
        """Reset agent state"""
        confirm = input(
            "Reset agent state? This will clear all facts and conversation history. (y/N): "
        )
        if confirm.lower() == "y":
            self.agent = create_integrated_agent()
        ...
        results.append(
            {
                ...
                "response": response,
                "duration": duration,
                "signals": {"X_G": signals.X_G, "X_F": signals.X_F, "X_T": signals.X_T},
            }
        )
