...
class BrutalQRFTTester:
    ...
    def test_mathematical_engine(self):
        ...
        try:
            ...
            self.log_result("Polynomial factorization", success, f"Length: {len(result)}")
    ...
    def test_paraconsistent_logic(self):
        ...
        try:
            ...
            self.log_result("Basic contradiction detection", len(contradictions) > 0, 
            ...
            contradictions = pc.contradictions()
            self.log_result("Nested contradictions", len(contradictions) >= 2,
                          f"Found {len(contradictions)} contradictions")
            
            # Test 3: X_G signal generation
            x_g = pc.x_g(alpha=0.5)
            self.log_result("X_G signal generation", 0 <= x_g <= 1,
                          f"X_G = {x_g:.3f}")
            ...
            self.log_result("Contradiction explosion resistance", x_g_massive > 0.9,
    ...
    def test_qrft_signals(self):
        ...
        try:
            ...
            x_g = agent.pc.x_g()
            self.log_result("X_G signal from contradictions", x_g > 0,
                          f"X_G = {x_g:.3f}")
            ...
            self.log_result("X_F novelty detection", 0 <= x_f <= 1,
                          f"X_F = {x_f:.3f}")
            
            # Test X_T (curl/complexity signal)
            complex_input = "Quantum mechanics meets recursive logic in nonlinear dynamics"
            x_t = agent.curl(complex_input)
            self.log_result("X_T complexity detection", 0 <= x_t <= 1,
                          f"X_T = {x_t:.3f}")
            
            # Test signal combination decision making
            signals = {"X_G": x_g, "X_F": x_f, "X_T": x_t}
            decision = agent.decision_threshold(signals)
            self.log_result("Signal-based decision making", decision in ['math', 'retrieve', 'plan', 'chat'],
                          f"Decision: {decision}")
    ...
    def test_deterministic_behavior(self):
        ...
        try:
            ...
            self.log_result("Deterministic behavior", all_identical,
            ...
            some_different = not all(r == results_diff_seeds[0] for r in results_diff_seeds)
            self.log_result("Seed variation", some_different,
    ...
    def test_edge_cases(self):
        ...
        try:
            ...
            try:
                ...
                self.log_result("Empty input handling", True, f"Handled gracefully: {type(result)}")
            except Exception as e:
            ...
            try:
                ...
                self.log_result("Massive input handling", True, f"Processed {len(massive_input)} chars")
            except Exception as e:
    ...
    def test_performance_benchmarks(self):
        ...
        try:
            ...
            self.log_result("Math solving speed", math_speed > 10,
                          f"{math_speed:.1f} problems/sec")
            ...
            self.log_result("Contradiction detection speed", contradiction_speed > 100,
            ...
            self.log_result("Memory efficiency", memory_mb < 500,
