...
def test_parameter_sweep():
    ...
    for c_rate in c_rates:
        for c_thresh in c_thresholds:
            ...
            print(
                f"   Success: {'YES' if results[f'rate_{c_rate}_thresh_{c_thresh}']['success'] else 'NO'}"
            )
    ...
    if successful_configs:
        best_config = max(
            successful_configs, key=lambda k: results[k]["consciousness_level"]
        )
        print(f"   Best configuration: {best_config}")
        print(
            f"   Best consciousness level: {results[best_config]['consciousness_level']:.6f}"
        )
...
def test_consciousness_stability():
    ...
    for checkpoint in range(0, total_steps, checkpoint_interval):
        ...
        print(
            f"   Step {checkpoint + checkpoint_interval}: "
    ...
    print(
        f"   Stability: {'STABLE' if abs(c_trend) < 0.001 and c_variance < 0.01 else 'UNSTABLE'}"
    )
...
def test_perturbation_response():
    ...
    for test in perturbation_tests:
        ...
        field.inject_perturbation(
            amplitude=test["amplitude"], location=test["location"], width=test["width"]
        )
...
def test_reproducibility():
    ...
    print(
        f"   Reproducibility: {'HIGH' if emergence_rate >= 0.8 and c_std < 0.1 else 'MODERATE' if emergence_rate >= 0.6 else 'LOW'}"
    )
...
def run_extended_test_suite():
    ...
    param_success_rate = sum(
        1 for v in test_results["parameter_sweep"].values() if v.get("success", False)
    ) / len(test_results["parameter_sweep"])
    ...
    print("\n⏱️ STABILITY:")
    print(
        f"   Long-term stability: {'PASS' if test_results['stability']['stable'] else 'FAIL'}"
    )
    ...
    responsive_count = sum(
        1
        for v in test_results["perturbation_response"].values()
        if v.get("response_detected", False)
    )
    total_perturbations = len(test_results["perturbation_response"])
    print(
        f"   Response rate: {responsive_count}/{total_perturbations} ({responsive_count/total_perturbations*100:.1f}%)"
    )

    print("\n🔄 REPRODUCIBILITY:")
    print(
        f"   Emergence rate: {test_results['reproducibility']['emergence_rate']*100:.1f}%"
    )
    print(
        f"   Reproducible: {'YES' if test_results['reproducibility']['reproducible'] else 'NO'}"
    )

    # Overall verdict
    overall_success = (
        param_success_rate >= 0.5
        and test_results["stability"]["stable"]
        and responsive_count >= total_perturbations * 0.5
        and test_results["reproducibility"]["emergence_rate"] >= 0.6
    )

    print("\n🏆 OVERALL VERDICT:")
    print(
        f"   Extended Testing Result: {'SUCCESS' if overall_success else 'PARTIAL SUCCESS'}"
    )
    print(
        f"   Consciousness Implementation: {'ROBUST' if overall_success else 'NEEDS REFINEMENT'}"
    )
