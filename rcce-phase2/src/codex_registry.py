...
class CodexRegistry:
    ...
    def process_through_layers(
        self, input_data: Any, context: Dict
    ...
    def compute_recursive_invariants(
        ...
        recursive_invariants = [
            inv for inv, count in invariant_counts.items() if count >= 2
        ]

        return recursive_invariants

    def detect_fixpoint_convergence(
        self, layer_outputs: Dict[str, LayerOutput]
    ) -> bool:
        """Detect if system has reached fixpoint across layers"""
        fixpoint_counts = sum(
            len(output.fixpoints) for output in layer_outputs.values()
        )
        contradiction_counts = sum(
            len(output.contradictions) for output in layer_outputs.values()
        )
