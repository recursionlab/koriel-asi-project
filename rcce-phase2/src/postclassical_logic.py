...
class PostClassicalLogic:
    ...
    def add_proposition(
        self, prop_id: str, content: str, truth_value: TruthValue = TruthValue.UNKNOWN
    ):
    ...
    def detect_contradictions(self) -> List[Contradiction]:
        ...
        for i, (id_a, node_a) in enumerate(prop_items):
            for id_b, node_b in prop_items[i + 1 :]:
                ...
                if (
                    node_a.truth_value == TruthValue.TRUE
                    and node_b.truth_value == TruthValue.FALSE
                    and self._propositions_related(
                        node_a.proposition, node_b.proposition
                    )
                ):
    ...
    def harvest_contradiction_energy(
        ...
        for contradiction in contradictions:
            ...
            if contradiction.source_a in self.propositions:
                ...
                self.propositions[contradiction.source_a].truth_value = (
    ...
    def generate_lambda_candidates(self, contradiction_energy: float) -> List[str]:
        ...
        if contradiction_energy > 1.0:
            ...
            candidates.extend(
                ["partial_negation", "perspective_shift", "recursive_deepening"]
            )
        else:
    ...
    def compute_recursive_invariants(
        self, proposition_history: List[Dict]
    ) -> List[str]:
    ...
    def contradiction_voltage_analysis(self) -> Dict[str, float]:
        ...
        for contradiction in self.contradictions:
            ...
            distribution[ctype] = (
                distribution.get(ctype, 0.0) + contradiction.energy_level
            )
