...
EXCLUDE_DIRS = {"venv", ".venv", ".git", "__pycache__", "artifacts", "checkpoints", "docs", "tests", "benchmarks", "data"}
...
def is_trivial_script(path: Path, max_kb: int = 40, max_defs: int = 3) -> Tuple[bool, str]:
...
def get_top_level_imports(repo_root: Path, search_paths: List[Path]) -> Set[str]:
    ...
    stdlib_like = {"os", "sys", "pathlib", "json", "typing", "ast", "subprocess", "shutil", "re", "math", "itertools", "collections", "datetime", "time", "logging", "glob", "inspect", "dataclasses"}
...
def run_compile_check(repo_root: Path) -> List[str]:
    ...
    out = []
    try:
        proc = subprocess.run(["python3", "-m", "compileall", "-q", str(repo_root)], capture_output=True, text=True)
...
def suggest_actions(repo_root: Path) -> Dict[str, List[str]]:
    ...
    suggestions["move_to_legacy"] = [f"{p.relative_to(repo_root)} ({reason})" for p, reason in trivial_files]
...
def main(argv: List[str] | None = None) -> int:
    ...
    parser.add_argument("--apply", action="store_true", help="Apply moves (non-reversible) to tools/legacy/")
    parser.add_argument("--summary", action="store_true", help="Print a summary report (default)")
