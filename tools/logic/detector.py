...
def _normalize_literal(lit: Any) -> Tuple[bool, str]:
    ...
    if isinstance(lit, tuple) and len(lit) == 2:
        pol = bool(lit[0]); atom = str(lit[1]).strip(); return pol, atom
    if isinstance(lit, str):
        s = lit.strip(); pol = True; lowered = s.lower()
        if lowered.startswith("not "):
            pol = False; s = s[4:].strip()
        elif s.startswith("¬"):
            pol = False; s = s[1:].strip()
...
def detect(clauses: List[Any]) -> Dict[str, Any]:
    pos: Dict[str, List[int]] = {}; neg: Dict[str, List[int]] = {}; atoms = set()
